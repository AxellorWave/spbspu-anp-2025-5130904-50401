...
int* init_matrix(const char* filename, MemoryType type, int* rows, int* cols, int** buffer, bool* is_dynamic) {
...
int* lft_bot_ctn(int* buffer, int rows, int cols) {
    ...
    while (i <= rows * cols) {
        ...
        switch (going_mode) {
            case 0: can_we_go_this_way = x + 1 < (cols - border_padding[0]); break;
            case 1: can_we_go_this_way = y - 1 > border_padding[1] - 1; break;
            case 2: can_we_go_this_way = x - 1 > border_padding[2] - 1; break;
            case 3: can_we_go_this_way = y + 1 < rows - border_padding[3]; break;
        }
        if (!can_we_go_this_way) {
            switch (going_mode) {
                case 0: going_mode = 1; border_padding[0]++; break;
                case 1: going_mode = 2; border_padding[1]++; break;
                case 2: going_mode = 3; border_padding[2]++; break;
                case 3: going_mode = 0; border_padding[3]++; break;
            }
            continue;
        }
        arr[cols * y + x] += i;
...
void write_result(const char* filename, int rows, int cols, int* result, int best_col) {
